...
static const WDC_REG gNEVISPCI_CfgRegs[] = {
...
static const WDC_REG gNEVISPCI_ext_CfgRegs[] = {
...
#define NEVISPCI_CFG_EXT_REGS_NUM sizeof(gNEVISPCI_ext_CfgRegs) / sizeof(WDC_REG)
...
static void DiagIntHandler(WDC_DEVICE_HANDLE hDev, NEVISPCI_INT_RESULT *pIntResult);
 ...
 *************************************************************/
int main_not_used(void)
{
    WDC_DEVICE_HANDLE hDev = NULL;
    DWORD dwStatus;

    printf("\n");
    printf("NEVISPCI diagnostic utility.\n");
    printf("Application accesses hardware using " WD_PROD_NAME "\n");

    /* Initialize the NEVISPCI library */
    dwStatus = NEVISPCI_LibInit();
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        NEVISPCI_ERR("nevispci_diag: Failed to initialize the NEVISPCI library: %s",
            NEVISPCI_GetLastErr());
        return dwStatus;
    }
    ...
    if (NEVISPCI_DEFAULT_VENDOR_ID)
        hDev = DeviceFindAndOpen(NEVISPCI_DEFAULT_VENDOR_ID, NEVISPCI_DEFAULT_DEVICE_ID);
    ...
    dwStatus = NEVISPCI_LibUninit();
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        NEVISPCI_ERR("nevispci_diag: Failed to uninitialize the NEVISPCI library: %s",
            NEVISPCI_GetLastErr());
    }
...
static WDC_DEVICE_HANDLE DeviceOpen(const WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        NEVISPCI_ERR("DeviceOpen: Failed retrieving the device's resources "
...
static void DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (!NEVISPCI_DeviceClose(hDev))
        NEVISPCI_ERR("DeviceClose: Failed closing PCI device: %s", NEVISPCI_GetLastErr());
}
...
static void MenuReadWriteAddr(WDC_DEVICE_HANDLE hDev)
{
    ...
    {
        DWORD dwNumAddrSpaces = NEVISPCI_GetNumAddrSpaces(hDev);
        ...
        {
            NEVISPCI_ERR("MenuReadWriteAddr: Error - No active address spaces "
                "found\n");
...
static void SetAddrSpace(WDC_DEVICE_HANDLE hDev, PDWORD pdwAddrSpace)
{
    DWORD dwAddrSpace;
    DWORD dwNumAddrSpaces = NEVISPCI_GetNumAddrSpaces(hDev);
    NEVISPCI_ADDR_SPACE_INFO addrSpaceInfo;
    ...
    for (dwAddrSpace = 0; dwAddrSpace < dwNumAddrSpaces; dwAddrSpace++)
    {
        ...
        if (!NEVISPCI_GetAddrSpaceInfo(hDev, &addrSpaceInfo))
        {
            NEVISPCI_ERR("SetAddrSpace: Error - Failed to get address space "
                "information. Last error [%s]", NEVISPCI_GetLastErr());
            return;
        }
...
static void MenuReadWriteCfgSpace(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        printf("------------------------\n");
        WDC_DIAG_RegsInfoPrint(hDev, gNEVISPCI_CfgRegs, NEVISPCI_CFG_REGS_NUM,
            WDC_DIAG_REG_PRINT_ALL & ~WDC_DIAG_REG_PRINT_ADDR_SPACE, FALSE);
        if (fExpress)
        {
            WDC_DIAG_RegsInfoPrint(hDev, gNEVISPCI_ext_CfgRegs, NEVISPCI_CFG_EXT_REGS_NUM,
        ...
        if (fExpress)
        {
            printf("%d. Read from a named NEVISPCI Express register\n",
                MENU_RW_CFG_SPACE_EXT_READ_REG);
            printf("%d. Write to a named NEVISPCI Express register\n",
                MENU_RW_CFG_SPACE_EXT_WRITE_REG);
        }
        ...
        case MENU_RW_CFG_SPACE_READ_ALL_REGS:
            WDC_DIAG_ReadRegsAll(hDev, gNEVISPCI_CfgRegs, NEVISPCI_CFG_REGS_NUM,
                TRUE, FALSE);
            if (fExpress)
            {
                WDC_DIAG_ReadRegsAll(hDev, gNEVISPCI_ext_CfgRegs,
                    NEVISPCI_CFG_EXT_REGS_NUM, TRUE, TRUE);
            }
            break;
        case MENU_RW_CFG_SPACE_READ_REG:  /* Read from a configuration
                                             register */
            WDC_DIAG_ReadWriteReg(hDev, gNEVISPCI_CfgRegs, NEVISPCI_CFG_REGS_NUM,
                WDC_READ, TRUE);
            break;
        ...
        case MENU_RW_CFG_SPACE_EXT_WRITE_REG: /* Write to a configuration
                                             NEVISPCI Express register */
            WDC_DIAG_ReadWriteReg(hDev, gNEVISPCI_ext_CfgRegs, NEVISPCI_CFG_EXT_REGS_NUM,
                WDC_WRITE, TRUE);
            break;
...
static void MenuReadWriteRegs(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        printf("\n");
        printf("NEVISPCI run-time registers:\n");
        printf("--------------------------\n");
        WDC_DIAG_RegsInfoPrint(gpNEVISPCI_Regs, NEVISPCI_REGS_NUM, WDC_DIAG_REG_PRINT_ALL, FALSE, FALSE);

        printf("\n");
        printf("Read/write the NEVISPCI run-time registers\n");
        printf("-----------------------------------------\n");
        ...
        case MENU_RW_REGS_READ_ALL:
            WDC_DIAG_ReadRegsAll(hDev, gpNEVISPCI_Regs, NEVISPCI_REGS_NUM, FALSE, FALSE);
            break;
        case MENU_RW_REGS_READ_REG:  /* Read from a register */
            WDC_DIAG_ReadWriteReg(hDev, gpNEVISPCI_Regs, NEVISPCI_REGS_NUM, WDC_READ, FALSE);
            break;
        case MENU_RW_REGS_WRITE_REG: /* Write to a register */
            WDC_DIAG_ReadWriteReg(hDev, gpNEVISPCI_Regs, NEVISPCI_REGS_NUM, WDC_WRITE, FALSE);
            break;
        }
...
static void MenuInterrupts(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        fIntEnable = !NEVISPCI_IntIsEnabled(hDev);
        ...
        case MENU_INT_ENABLE_DISABLE: /* Enable/disable interrupts */
            if (fIntEnable)
            {
                DWORD dwStatus = NEVISPCI_IntEnable(hDev, DiagIntHandler);
                ...
                {
                    NEVISPCI_ERR("Failed enabling interrupts. Error [0x%lx - %s]\n",
                        dwStatus, Stat2Str(dwStatus));
                }
            }
            else
            {
                if (WD_STATUS_SUCCESS == NEVISPCI_IntDisable(hDev))
                {
                    printf("Interrupts disabled\n");
                }
                else
                {
                    NEVISPCI_ERR("Failed disabling interrupts: %s",
                        NEVISPCI_GetLastErr());
                }
            }
            break;
...
static void FreeDmaMem(PVOID *ppBuf, WD_DMA **ppDma)
{
    ...
    {
        NEVISPCI_ERR("Failed trying to free DMA memory. Error [0x%lx - %s]\n",
            dwStatus, Stat2Str(dwStatus));
    }
...
static void MenuDma(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        ...
        case MENU_DMA_ALLOCATE_CONTIG: /* Allocate contiguous memory */
            ...
            {
                NEVISPCI_ERR("Failed allocating contiguous memory. size [%ld], "
        ...
        case MENU_DMA_ALLOCATE_SG: /* Allocate scatter-gather memory */
            ...
            if (!pBuf)
            {
                NEVISPCI_ERR("Failed allocating user memory for SG. size [%ld]\n",
                    size);
                continue;
            }
            ...
            {
                NEVISPCI_ERR("Failed allocating SG memory. size [%ld], "
                    "Error [0x%lx - %s]\n", size, dwStatus, Stat2Str(dwStatus));
                free(pBuf);
            }
            break;
        ...
        case MENU_DMA_RESERVED_MEM:
            ...
            {
                NEVISPCI_ERR("Failed claiming reserved memory. size [%ld], "
                    "Error [0x%lx - %s]\n", size, dwStatus, Stat2Str(dwStatus));
            }
            break;
...
static void MenuEvents(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        fRegister = !NEVISPCI_EventIsRegistered(hDev);
        ...
        case MENU_EVENTS_REGISTER_UNREGISTER: /* Register/unregister events */
            if (fRegister)
            {
                if (WD_STATUS_SUCCESS == NEVISPCI_EventRegister(hDev,
                    DiagEventHandler))
                {
                ...
                {
                    NEVISPCI_ERR("Failed to register events. Last error [%s]",
                        NEVISPCI_GetLastErr());
                }
            }
            else
            {
                if (WD_STATUS_SUCCESS == NEVISPCI_EventUnregister(hDev))
                {
                    printf("Events unregistered\n");
                }
                else
                {
                    NEVISPCI_ERR("Failed to unregister events. Last error [%s]",
                        NEVISPCI_GetLastErr());
                }
            }
            break;
