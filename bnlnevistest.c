...
int main(void)
{
  ...
  if (WD_STATUS_SUCCESS != dwStatus)
    {
      NEVISPCI_ERR("pcie_diag: Failed to initialize the NEVISPCI library: %s",
	       NEVISPCI_GetLastErr());
      return dwStatus;
    }
  
  /* Find and open a NEVISPCI device (by default ID) */
  if (NEVISPCI_DEFAULT_VENDOR_ID)
    hDev = DeviceFindAndOpen(NEVISPCI_DEFAULT_VENDOR_ID, NEVISPCI_DEFAULT_DEVICE_ID);
  if (NEVISPCI_DEFAULT_VENDOR_ID)
    hDev1 = DeviceFindAndOpen(NEVISPCI_DEFAULT_VENDOR_ID, NEVISPCI_DEFAULT_DEVICE_ID+1);
  if (NEVISPCI_DEFAULT_VENDOR_ID)
    hDev2 = DeviceFindAndOpen(NEVISPCI_DEFAULT_VENDOR_ID, NEVISPCI_DEFAULT_DEVICE_ID+2);
  ...
  if (WD_STATUS_SUCCESS != dwStatus)
    NEVISPCI_ERR("pcie_diag: Failed to uninit the NEVISPCI library: %s", NEVISPCI_GetLastErr());
...
//    DWORD dwOptions_rec = DMA_FROM_DEVICE | DMA_ALLOW_CACHE | DMA_ALLOW_64BIT_ADDRESS;
    ...
    */
    ...
    for(j=0; j<nloop; j++){
      ...
      //scanf("%d", &aux);
      usleep(10000); // wait for 10ms
      inpf = fopen("/home/nfs/sbnd/jcrespo/VST/sbnddaq3/srcs/sbnddaq_readout/sbnddaq-readout/Generators/NevisTPC/nevishwutils/fpga/feb_fpga_lar1nd","r");
      imod=imod_fem;
...
static void MenuReadWriteAddr(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (ACTIVE_ADDR_SPACE_NEEDS_INIT == dwAddrSpace)
    {
        ...
        {
            NEVISPCI_ERR("MenuReadWriteAddr: Error - no active address spaces found\n");
...
static void SetAddrSpace(WDC_DEVICE_HANDLE hDev, PDWORD pdwAddrSpace)
{
    ...
    for (dwAddrSpace = 0; dwAddrSpace < dwNumAddrSpaces; dwAddrSpace++)
    {
        ...
        {
            NEVISPCI_ERR("SetAddrSpace: Error - Failed to get address space information: %s",
                NEVISPCI_GetLastErr());
            return;
        }
...
/*         if (NEVISPCI_CFG_REGS_NUM) */
/*         { */
/*             printf("\n"); */
/*             printf("Configuration registers:\n"); */
/*             printf("------------------------\n"); */
/*             WDC_DIAG_RegsInfoPrint(gpNEVISPCI_CfgRegs, NEVISPCI_CFG_REGS_NUM, */
...
/*         if (DIAG_INPUT_FAIL == DIAG_GetMenuOption(&option, */
/*             NEVISPCI_CFG_REGS_NUM ? MENU_RW_CFG_SPACE_WRITE_REG : */
/*             MENU_RW_CFG_SPACE_WRITE_OFFSET)) */
/*         { */
/*             continue; */
/*         } */
...
/*             WDC_DIAG_ReadRegsAll(hDev, gpNEVISPCI_CfgRegs, NEVISPCI_CFG_REGS_NUM, TRUE); */
/*             break; */
/*         case MENU_RW_CFG_SPACE_READ_REG:  /\* Read from a configuration register *\/ */
/*             WDC_DIAG_ReadWriteReg(hDev, gpNEVISPCI_CfgRegs, NEVISPCI_CFG_REGS_NUM, WDC_READ, TRUE); */
/*             break; */
/*         case MENU_RW_CFG_SPACE_WRITE_REG: /\* Write to a configuration register *\/ */
/*             WDC_DIAG_ReadWriteReg(hDev, gpNEVISPCI_CfgRegs, NEVISPCI_CFG_REGS_NUM, WDC_WRITE, TRUE); */
/*             break; */
/*         } */
...
/*         printf("--------------------------\n");    */
/*         WDC_DIAG_RegsInfoPrint(gpNEVISPCI_Regs, NEVISPCI_REGS_NUM, WDC_DIAG_REG_PRINT_ALL); */
...
/*             WDC_DIAG_ReadRegsAll(hDev, gpNEVISPCI_Regs, NEVISPCI_REGS_NUM, FALSE); */
/*             break; */
...
static void MenuInterrupts(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (dwIntOptions & INTERRUPT_LEVEL_SENSITIVE)
    {
        /* TODO: You can remove this message after you have modified the
           implementation of NEVISPCI_IntEnable() in pcie_lib.c to 
        ...
        printf("Before enabling the interrupts, %s first modify the source "
           "code of NEVISPCI_IntEnable(), in the file pcie_lib.c, to "
        ...
        case MENU_INT_ENABLE_DISABLE: /* Enable/disable interrupts */
            ...
            else
            {
                if (WD_STATUS_SUCCESS == NEVISPCI_IntDisable(hDev))
                    ...
                    NEVISPCI_ERR("Failed disabling interrupts: %s", NEVISPCI_GetLastErr());
            }
            break;
        }
...
static void MenuEvents(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        ...
        case MENU_EVENTS_REGISTER_UNREGISTER: /* Register/unregister events */
            if (fRegister)
            {
                if (WD_STATUS_SUCCESS == NEVISPCI_EventRegister(hDev, DiagEventHandler))
                    printf("Events registered\n");
                else
                    NEVISPCI_ERR("Failed to register events. Last error:\n%s", NEVISPCI_GetLastErr());
            }
            else
            {
                if (WD_STATUS_SUCCESS == NEVISPCI_EventUnregister(hDev))
                    ...
                    NEVISPCI_ERR("Failed to unregister events. Last Error:\n%s", NEVISPCI_GetLastErr());
            }
            break;
        }
